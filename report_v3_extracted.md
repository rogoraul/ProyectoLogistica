We built and tested two methods for the Maximum Diversity Problem: a
baseline GRASP and a GRASP combined with Tabu Search. Our main result is
slightly counterintuitive: under equal time budgets, the more complex
method did not outperform the simpler one on average. We do not
interpret that as a failure of the project. We interpret it as a
meaningful experimental result. In this report we explain why we think
that happened, which implementation decisions made the comparison fair,
which programming difficulties shaped the final code, and what the data
suggest about when each method has an advantage.
...
logic. We split data loading, solution handling, constructive search,
local improvement, timed algorithm wrappers, and experiment scripts into
different modules so that we could modify one part without silently
...
One of our most useful decisions was building the full symmetric matrix
at load time. We did that because later phases need constant-time
distance lookups again and again during construction and swap
evaluation. We did not want each algorithm to parse or reinterpret the
benchmark format independently, and we did not want distance access to
become a hidden bottleneck inside the search.

We also avoided duplicating algorithm code inside the experiment
scripts. The calibration script imports the same algorithm modules that
the comparison script later imports. That mattered to us because
duplicated search code tends to drift over time and makes the final
report much harder to trust. By forcing both experiment stages to use
the same implementations, we could argue that differences in the results
came from parameters and search behavior, not from inconsistent code
copies.

Finally, we exported the outputs systematically to CSV and JSON. We did
not want the project to depend on reading console output by hand. The
exported files became the backbone of our analysis because they
preserved per-configuration data, per-run data, and the final summaries
that we later used to interpret the results and build the report.
...
These data-structure choices were not cosmetic. They directly determined
how many restarts and neighborhood scans we could complete under the
same time budget. In that sense, they became part of the experimental
design and not just part of the implementation.
...
We ran all 15 benchmark instances from the MDG repository: 6 small
instances (n=100) and 9 large instances (n=500). We calibrated and
compared each group separately, because we expected the role of
parameters to change with scale, and it did.
...
This turned out to be the right choice. The small instances saturate
quickly and mainly work as a consistency check, while the large
instances preserve enough difficulty for alpha and tenure to have a
visible and meaningful effect.
...
metaheuristic becomes redundant.
  ...
  **Large-instance aggregate **GRASP**        **GRASP+TS**
  ...
  Wilcoxon p-value (paired   \-               0.002628
  test)                                       
  ------------------------------------------------------------

The p-value refers to the two-sided Wilcoxon signed-rank test comparing
both methods across the 45 paired runs on large instances.
...
sharply. MDG-a_13 shows the opposite side of the story. On MDG-a_13, the
mean deviation for GRASP was 0.5463% versus 0.4148% for GRASP+TS, so it
became one of the few large instances where the deeper search paid off
on average. We therefore do not think the correct interpretation is a
simplistic universal ranking. The result depends on whether we care more
about average quality or about occasional high peaks.
...
conclusions, but they do define their scope. In particular, our
sequential calibration (alpha first, then tenure) assumes low
interaction between parameters. If strong interaction exists, the chosen
combination may not be globally optimal, though the clarity of the
observed trends makes this unlikely to reverse our main conclusion.
